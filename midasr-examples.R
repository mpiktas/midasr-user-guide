...
midas_coef(eq.r)
mmweights(p=c(1,-0.5),d=8,m=4,weight=nealmon,type="C")
nealmon(p=c(1,-0.5),d=4)
## NLS using midas_r with aggregates
eq.r<-midas_r(y~trend+mls(x,0:7,4,mmweights,nealmon,"C")+mls(z,0:16,12,nealmon),start=list(x=c(1,-0.5),z=c(2,0.5,-0.1)))
summary(eq.r)
## Table 3-related NLS variations using midas_r (trend dropped in the table and can be omitted here)
fn<-gompertzp
eq.r1<-midas_r(y~trend+mls(x,0:7,4,nealmon)+mls(z,0:16,12,fn),start=list(x=c(1,-0.5),z=c(1,0.5,0.1)))
summary(eq.r1)
eq.r2<-midas_r(y~trend+mls(x,0:7,4)+mls(z,0:16,12,nealmon),start=list(z=c(1,-0.5)))
summary(eq.r2)
eq.r3<-midas_r(y~trend+mls(y,1:2,1)+mls(x,0:7,4,nealmon),start=list(x=c(1,-0.5)))
summary(eq.r3)
eq.r4<-midas_r(y~trend+mls(y,1:2,1,"*")+mls(x,0:7,4,nealmon),start=list(x=c(1,-0.5)))
summary(eq.r4)
eq.r5<-midas_r(y~trend+mls(y,1:4,1,nealmon)+mls(x,0:7,4,nealmon),start=list(y=c(1,-0.5),x=c(1,-0.5)))
summary(eq.r5)
eq.r6<-midas_r(y~trend+mls(x,0:7,4,mmweights,nealmon,"A"),start=list(x=c(1,1,1,-0.5)))
summary(eq.r6)
eq.r7<-midas_r(y~trend+mls(x,0:7,4,mmweights,nealmon,"B"),start=list(x=c(1,1,-0.5)))
summary(eq.r7)
eq.r8<-midas_r(y~trend+mls(x,0:7,4,mmweights,nealmon,"C"),start=list(x=c(1,-0.5)))
summary(eq.r8)
fn<-function(p,d){
   p[1]*c(1:d)^p[2]
}
...
eq.rb<-midas_r(y~trend+mls(x,0:7,4,nealmon)+mls(z,0:12,12,nealmon),start=list(x=c(1,-0.5),z=c(2,-0.1)))
hAh.test(eq.rb)
hAhr.test(eq.rb)
summary(eq.rb)
#######################
...
set.x<- expand_weights_lags(weights=c("nealmon","almonp"),from=0,to=c(5,10),
m=1,start=list(nealmon=rep(1,2),almonp=rep(1,3)))
set.z <- expand_weights_lags(c("nealmon","nealmon"),0,c(10,20),1,start=list(nealmon=rep(1,2),nealmon=rep(1,3)))
expand_weights_lags(weights=c("nealmon","nbeta"),from=1,to=c(2,3),m=1,start=list(nealmon=rep(0,2),nbeta=rep(0.5,3)))
## Estimation and selection of models
eqs.ic<-midas_r_ic_table(y~trend+mls(x,0,m=4)+fmls(z,0,m=12), table=list(z=set.z,x=set.x),start=c("(Intercept)"=0,trend=0))
mod<-modsel(eqs.ic,IC="AIC",type="restricted")
#############################
### Forecast combinations ###
## With model selection and horizon 1:3 
cbfc<-select_and_forecast(y~trend+mls(x,0,m=4)+mls(z,0,m=12),
...
nealmon2<-nealmon
nealmon3<-nealmon
cbfc1 <-select_and_forecast(y~trend+mls(x,0,4)+mls(z,0,12),
from=list(x=c(4),z=c(12)),
to=list(x=rbind(c(14,14)),z=rbind(c(22,22))),
insample=1:200,outsample=201:250,
weights=list(x=c("nealmon3"),z=c("nealmon2")),
wstart=list(nealmon3=c(10,1,-0.1),nealmon2=c(2,-0.1)),
IC="AIC",
seltype="restricted",
ftype="fixed",                            
measures=c("MSE","MAPE","MASE"),
...
###Do a model preselection individualy
...
mod1 <- midas_r(y ~ trend + mls(x, 4:14, 4, nealmon3) + mls(z, 12:22, 12, nealmon2),data=datasplit$indata,start=list(x=c(10,1,-0.1),z=c(2,-0.1)))

mod2 <- midas_r(y ~ trend + mls(x, 4:20, 4, nealmon3) + mls(z, 12:25, 12, nealmon2),data=datasplit$indata,start=list(x=c(10,1,-0.1),z=c(2,-0.1)))
...
##Produce rolling forecasts, where for each forecast models are reestimated using rolling window
avgrollf <- average_forecast(list(mod1,mod2),
                         data=list(y=y,x=x,z=z,trend=trend),
                         insample=1:200,outsample=201:210,
                         type="rolling",                            
                         measures=c("MSE","MAPE","MASE"),
                         fweights=c("EW","BICW","MSFE","DMSFE"))
...
##Produce recursive forecasts where for each forecast models are reestimated by recursively increasing estimation sample
avgrecf <- average_forecast(list(mod1,mod2),
                         data=list(y=y,x=x,z=z,trend=trend),
                         insample=1:200,outsample=201:210,
                         type="recursive",                            
                         measures=c("MSE","MAPE","MASE"),
...
## Calculate the individual forecasts of each of the model and
