...
eq.r<-midas_r(y~trend+mls(x,0:7,4,nealmon)+mls(z,0:16,12,nealmon),
...
eq.r<-midas_r(y~trend+mls(x,0:7,4,nealmon)+mls(z,0:16,12,nealmon),
              start=list(x=c(1,-0.5),z=c(2,0.5,-0.1)),
              Ofunction="optim",method="Nelder-Mead")
eq.r<-midas_r(eq.r,Ofunction="nls")
@ 
...
<<echo=TRUE,cache=TRUE, results="hide">>=
eq.r<-midas_r(y~trend+mls(x,0:7,4,amweights,nealmon,"C")+mls(z,0:16,12,nealmon),start=list(x=c(1,-0.5),z=c(2,0.5,-0.1)))
@ 
