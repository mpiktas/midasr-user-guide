...
The key feature of the package is its flexibility in terms of the model formulation and estimation, which allows for the\footnote{
 \cite{ghysels:2013} also developed a package for \proglang{MATLAB} which deals with the estimation and information criteria-based specification of MIDAS regressions as well as forecasting and nowcasting of slow frequency series.  All of the listed features replicate or extend features provided by the said package. The key extensions of package \pkg{midasr} are: the ability to specify any user-defined functional contraints, the ability to include multiple variables of different frequency and different functional constraints, and the ability to test the adequacy of a chosen model specification.}:
\begin{itemize}
...
The model stated in the equation \eqref{l:eqintro} can be estimated in the usual time series regression fashion or using a Bayesian approach. However, the number of parameters in this model $d$ = $p+\sum_{i=0}^kl_i$ can be very large in terms of the number $n$ of available observations of $y_t$\footnote{In the MIDAS literature it is common to have $k_i\ge m_i$ and $m_i$ can be large, for example monthly versus daily data}. Since the estimation of the model can
...
<<echo=TRUE,cache=FALSE>>=
mr <- midas_r(rv~mls(rv,1:20,1,nealmon), data=list(rv=spx2_rvol),start=list(rv=c(0,0,0)), 
...
mtb <- update(mtb)
@
Here we used two optimization methods to improve the convergence. The function \code{midas_r_ic_table} applies the \code{test} function for each  candidate model. The function \code{hAhr_test} takes a lot of computing time, especialy for models with larger number of lags, so we calculate it only for the second final step, and we restrict the number of lags choose from.  The AIC selects the model with 9 lags:

<<echo=TRUE,cache=TRUE>>=
bm <- modsel(mtb)
@
The HAC robust version of \code{hAh_test} again does not reject the null
