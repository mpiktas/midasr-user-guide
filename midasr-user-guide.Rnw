...
data("oos_prec")
qplot(x=n,y=value,data=oos_prec,geom="line",colour=Constraint,ylab="")+facet_wrap(~Type,scales="free_y")+xlab("Sample size")
@
...
\end{equation}
where $\yy_{t,0}=(y_{t},\dots,y_{t-p+1})'$ and $\alal=(\alpha_{1},\alpha_{2},\dots,\alpha_{p})^\prime$ is a vector of parameters of the autoregressive terms. This representation is well suited for (one step ahead) conditional forecasting of $y_{t+1},$ provided that the information on the explanatory variables is available. If it were absent, forecasts of $\xx_{t+1,0}$ would be also necessary from a joint process of $\{y_t,\xx_{t,0}\}$ which might be difficult to specify and estimate correctly, especially, bearing in mind the presence of data with mixed frequencies. Instead, a direct approach to forecasting is often applied in the MIDAS framework. Namely, given an information set available up to a moment $t$ defined by $\mathcal{I}_{t,0}=\{\yy_{t,j},\xx_{t,j}\}_{j=0}^{\infty},$ where 
\begin{align*}
 \yy_{t,j}&=(y_{t-j},...,y_{t-j-p+1})'\\
 \xx_{t,j}&=(x^{(0)}_{tm_0},...,x^{(i)}_{tm_i},...,x^{(h)}_{tm_h})',
\end{align*}
...
\caption{\small A summary of a basic data handling functionality in the package \emph{midasr}.}
\label{tab:2}
\end{center}
\end{table}
%\end{sidewaystable}
...
@ 
\caption{Shapes of impact, blue for $x_\tau$, red for $z_\tau$.}
\label{fig:fig2}
\end{figure}
...
@ 
The naming convention \code{restriction_name.gradient} is important, currently this is the way the \code{midas_r} ``knows'' about the gradient functions. To use this function for the optimization it is necessary to set \code{user.gradient=TRUE} in call to \code{midas_r}:
<<eval=TRUE,echo=TRUE,cache=TRUE,results="markup",tidy=FALSE>>=
eq.r2 <- midas_r(y ~ trend + mls(x, 0:7, 4, nealmon) + 
                 mls(z, 0:16, 12, nealmon),
                 start = list(x = c(1, -0.5), z = c(2, 0.5, -0.1)),
...
@ 
s
To retrieve a vector of constrained estimates  $\tilde\theta$ (and, hence, also $\hat\ff=\ff_{\gaga}\big|_{{\gaga}=\widehat{\gaga}}$) which.min corresponds to the vector ${\thth}$ ({\bebe}, respectively), the function \code{midas_coef} can be used as follows
<<echo=TRUE,cache=TRUE, results="markup">>=
midas_coef(eq.r)
@ 
...
\caption{\small Some functionality of the specification of MIDAS regressions in \emph{midasr} package.}
\label{tab:3}
\end{center}
Note: A the function \code{fn} in the last example of the table is defined e.g., by \verb|fn<-function(p,d){p[1]*c(1:d)^p[2]}|
...
The other options of the function \code{select_and_forecast} do rather not require further explanation

<<eval = FALSE, echo = TRUE, cache = TRUE, results = "hide", tidy = FALSE>>=
cbfc <- select_and_forecast(y ~ trend + mls(x, 0, 4) + mls(z, 0, 12),
                            from = list(x = c(4, 8, 12), 
                                        z = c(12, 24, 36)),
                            to = 
                            list(x = rbind(c(14, 19), c(18, 23), c(22, 27)),
                                 z = rbind(c(22, 27), c(34, 39), c(46, 51))),
                            insample = 1:200, outsample = 201:250,
                            weights = list(x = c("nealmon", "almonp"),
                                           z = c("nealmon", "almonp")),
                            wstart = list(nealmon = rep(1, 3), 
                                          almonp = rep(1, 3)),
                            IC = "AIC", seltype = "restricted", 
                            ftype = "fixed",                          
...
lines(ny, col = 2)
@ 
\caption{Quaterly gross domestic product and monthly non-farm payroll employment growth rates.}
\label{fig:ghysels}
\end{figure}
...
<<echo = TRUE, cache = FALSE, results = "markup", tidy = FALSE, label = "rv_avgf">>=
