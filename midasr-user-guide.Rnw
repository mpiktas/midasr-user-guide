...
@ 
The naming convention \code{restriction_name.gradient} is important, currently this is the way gradient functions are supplied to \code{midas_r}. To use this function for the optimization it is necessary to set \code{user.gradient=TRUE} in call to \code{midas_r}:
<<eval=TRUE,echo=TRUE,cache=TRUE,results="markup",tidy=FALSE>>=
eq.r2 <- midas_r(y ~ trend + mls(x, 0:7, 4, nealmon) + 
                 mls(z, 0:16, 12, nealmon),
                 start = list(x = c(1, -0.5), z = c(2, 0.5, -0.1)),
