...
@ 
The naming convention \code{restriction_name.gradient} is important, since currently this is the way the \code{midas_r} ``knows'' about the gradient functions. To use this function for the optimisation it is necessary to set \code{user.gradient=TRUE} in call to the \code{midas_r}:
<<eval=TRUE,echo=TRUE,cache=TRUE,results="markup",tidy=FALSE>>=
eq.r2<-midas_r(y~trend+mls(x,0:7,4,nealmon)+mls(z,0:16,12,nealmon),
              start=list(x=c(1,-0.5),z=c(2,0.5,-0.1)),
              user.gradient=TRUE)
...
To retrieve the vector of constrained estimates  $\tilde\theta$ (and, hence, also $\hat\ff=\ff_{\gaga}\big|_{{\gaga}=\widehat{\gaga}}$) that corresponds to the vector ${\thth}$ ({\bebe}, respectively), the function \code{midas_coef} can be used as follows
<<echo=TRUE,cache=TRUE, results="markup">>=
midas_coef(eq.r)
@ 
...
@
Here we used two optimization methods to improve the convergence.
