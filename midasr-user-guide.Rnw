...
where we require
\begin{align*}
    E(\eps_t|y_{t-1},\dots,y_{t-p},x^{(0)}_{tm_0},\dots,x^{(0)}_{tm_0-l_i},\dots,x^{(h)}_{tm_h},\dots,x^{(h)}_{tm_k-l_k})=0,
...
eq_r2 <- midas_r(y ~ trend + mls(x, 0:7, 4, nealmon) + 
                 ...
                 start = list(x = c(1, -0.5), z = c(2, 0.5, -0.1)),
                 Ofunction = "optim", method = "Nelder-Mead")
eq_r2 <- midas_r(eq_r2, Ofunction = "nls")
@ 
...
eqs.ic<-midas_r_ic_table(y~trend+mls(x,0,m=4)+fmls(z,0,m=12),table=list(z=set_z,x=set_x))
@ 
